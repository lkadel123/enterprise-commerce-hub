...
import { CybersourceTokenError, decodeJwtPayload, extractClientLibrary } from "./cybersource-token.js";
...
export function buildCaptureContextRequest(input: CybersourceCaptureContextInput) {
    ...
    if (input.billing?.line1)
        billTo.address1 = input.billing.line1;
    if (input.billing?.line2)
        billTo.address2 = input.billing.line2;
    if (input.billing?.city)
        billTo.locality = input.billing.city;
    if (input.billing?.state)
        billTo.administrativeArea = input.billing.state;
    if (input.billing?.postalCode)
        billTo.postalCode = input.billing.postalCode;
    if (input.billing?.country) {
        ...
        if (isoCountry) {
            ...
            logger.warn({ merchantReference: input.merchantReference, country: input.billing.country }, "Cybersource billTo.country omitted: not an ISO-3166 alpha-2 code or known country name.");
        }
    }
    if (input.customer?.email)
        billTo.email = input.customer.email;
    if (input.customer?.firstName)
        billTo.firstName = input.customer.firstName.slice(0, 60);
    if (input.customer?.lastName)
        billTo.lastName = input.customer.lastName.slice(0, 60);
    ...
    if (targetOrigins.some((origin) => !origin.startsWith("https:"))) {
        ...
        logger.warn({ targetOrigins }, "Cybersource target origins contain non-HTTPS values; the sessions API requires HTTPS origins.");
    }
    const request = {
        targetOrigins,
...
export async function createCaptureContext(
    ...
    logger.info({ merchantReference: input.merchantReference, currency: request.data.orderInformation.amountDetails.currency }, "Creating Cybersource capture context");
    ...
    if (!captureContext || captureContext.split(".").length !== 3) {
        logger.error({ merchantReference: input.merchantReference }, "Cybersource session response malformed");
        throw new CybersourceTokenError("Cybersource session response did not contain a capture context.");
    }
    ...
    try {
        ...
        logger.error({ merchantReference: input.merchantReference, error }, "Capture context client library extraction failed");
        throw error;
    }
    logger.info({ merchantReference: input.merchantReference }, "Cybersource capture context created");
