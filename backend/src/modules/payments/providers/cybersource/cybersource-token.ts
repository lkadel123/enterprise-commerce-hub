...
export interface CybersourceTokenPayload {
  ...
  ctx?: Array<{
    type?: string;
    data?: { clientLibrary?: unknown; clientLibraryIntegrity?: unknown } & Record<string, unknown>;
  }>;
}
...
export function extractClientLibrary(payload: CybersourceTokenPayload): {
  ...
  if (Array.isArray(data)) {
    ...
    url = data;
    integrity =
      typeof entry?.data?.clientLibraryIntegrity === "string"
        ? entry.data.clientLibraryIntegrity
        : "";
  }
 ...
 * KEY NAME only.
 */
export function payloadKeyPaths(
  value: unknown,
  prefix = "",
  out: string[] = [],
  depth = 0,
...
export function tokenDiagnostics(payload: CybersourceTokenPayload): PaymentTokenDiagnostics {
  ...
  const paymentType =
    pickString(tokenizedCard, "paymentInstrumentType") ?? pickString(processing, "paymentSolution");
  ...
  return {
    ...
    authentication: {
      ...
      ...(pickString(consumerAuth, "authenticationTransactionId")
        ? {
            authenticationTransactionId: pickString(consumerAuth, "authenticationTransactionId")!,
          }
        : {}),
      ...(pickString(consumerAuth, "directoryServerTransactionId")
        ? {
            directoryServerTransactionId: pickString(consumerAuth, "directoryServerTransactionId")!,
          }
        : {}),
