...
const envSchema = z.object({
  ...
  ORDER_EXPIRY_SWEEP_INTERVAL_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(60_000),
  ...
  const fonepayCredentialsSet =
    [value.FONEPAY_BASE_URL, value.FONEPAY_USERNAME, value.FONEPAY_PASSWORD, value.FONEPAY_PRIVATE_KEY].filter(
      (part) => part.length > 0,
    ).length > 0;
...
if (env.NODE_ENV === "production") {
  ...
  const cybersourceOrigins = env.CYBERSOURCE_TARGET_ORIGINS
    ? env.CYBERSOURCE_TARGET_ORIGINS.split(",").map((o) => o.trim()).filter(Boolean)
