...
function UsersPage() {
  ...
  const updateUser = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) => usersApi.update(id, body as never),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
  ...
  const resetPassword = useMutation({
    ...
    onSuccess: () => {
      toast.success(`Password updated for ${resetTarget?.name ?? "user"}. Their active sessions were signed out.`);
      setResetTarget(null);
      setNewPassword("");
      setConfirmPassword("");
    },
  ...
  const roleAllows = (roleName: string, module: string, action: MatrixAction): boolean => {
    ...
    return role?.permissions.some((p) => p.module === module && (p.actions as string[]).includes(action)) ?? false;
  };
  ...
  return (
    <AppShell>
          ...
          <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Create an admin account with an initial password.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="text-xs">Full name</Label>
                  <Input className="h-9" value={name} onChange={(e) => setName(e.target.value)} placeholder="Jordan Ellis" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Work email</Label>
                  <Input type="email" className="h-9" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jordan.e@northpeak.com" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Initial password</Label>
                  <Input type="password" className="h-9" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimum 8 characters" />
                </div>
                <div className="space-y-1.5">
              ...
              <DialogFooter>
                <Button
                  size="sm"
                  disabled={createUser.isPending || !name.trim() || !email.trim() || !password || !role}
                  onClick={() => createUser.mutate({ name: name.trim(), email: email.trim(), password, role })}
                >
                  Create account
                </Button>
              </DialogFooter>
      ...
      <Section title="Admin users" description={usersQuery.isPending ? "Loading…" : `${users.length} accounts`} bodyClassName="p-0">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {usersQuery.isPending ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">Loading users…</td></tr>
              ) : usersQuery.isError ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-destructive">Couldn't load users. Check your connection and try again.</td></tr>
              ) : users.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">No admin accounts yet.</td></tr>
              ) : (
              users.map((u) => (
              ...
              <tr className="border-t">
                ...
                {modules.map((m) =>
                  ACTION_LABELS.map((a, i) => (
                    <th key={`${m}-${a}`} className={`px-2 py-1.5 text-center font-normal ${i === 0 ? "border-l" : ""}`}>{a.slice(0, 1).toUpperCase()}</th>
                  )),
                )}
              </tr>
            </thead>
            <tbody>
              {roles.map((role) => (
                <tr key={role.name} className="border-t hover:bg-surface-muted/40">
                  <td className="sticky left-0 bg-surface px-4 py-2.5 font-medium whitespace-nowrap">{role.name}</td>
                  {modules.map((m) =>
