  ...
  head: () => ({
    meta: [
      { title: "Inventory — Northpeak Commerce Console" },
      {
        name: "description",
        content:
          "Track stock levels, reserved units, incoming shipments and reorder thresholds by warehouse.",
      },
      { property: "og:title", content: "Inventory — Northpeak Commerce Console" },
      {
        property: "og:description",
        content: "Warehouse-level inventory control with low-stock alerts.",
      },
    ],
  }),
...
function InventoryPage() {
  ...
  const downloadTemplate = () => {
    const csv = [
      "sku,warehouse,delta,reason,note",
      "SKU-1001,Rotterdam DC,10,received,example row",
    ].join("\n");
    ...
    <AppShell>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <StatCard
          ...
          note="Tracked inventory records"
        />
        <StatCard
          label="In Stock"
          value={stats ? String(stats.inStock) : "—"}
        ...
        <StatCard
          ...
          value={stats ? formatNpr(stats.inventoryValue) : "—"}
          note="At cost price"
        />
      </div>
      ...
      <Section
        ...
        description="Automatically flagged at or below reorder level"
      >
        {rows.filter((p) => p.stock <= p.reorderLevel).length === 0 ? (
          <EmptyState
            title="No low-stock items"
            description="Everything is above its reorder level."
          />
        ) : (
                ...
                <li key={p.id} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="num truncate text-xs text-muted-foreground">
                      {p.sku} · {p.warehouse} · reorder at {p.reorderLevel}
                    </p>
                  </div>
      ...
      <Dialog open={adjusting !== null} onOpenChange={(open) => !open && setAdjusting(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {adjusting
                ? `${adjusting.productName ?? adjusting.sku} — ${adjusting.warehouse} (current: ${adjusting.stock})`
                : ""}
            </DialogDescription>
          </DialogHeader>
