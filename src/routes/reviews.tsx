  ...
  head: () => ({
    meta: [
      { title: "Reviews — Northpeak Commerce Console" },
      {
        name: "description",
        content:
          "Moderate product reviews, approve or reject submissions and monitor rating distribution.",
      },
...
function ReviewsPage() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="Reviews"
        description="Moderate customer feedback before it reaches the storefront."
      />

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Section
          title="Rating overview"
          description={stats ? `${total.toLocaleString()} approved reviews` : "Loading…"}
        >
          {stats ? (
            <>
              <div className="flex items-baseline gap-2">
                ...
                <span className="inline-flex">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${i <= Math.round(stats.average) ? "fill-warning text-warning" : "text-border"}`}
                    />
                  ))}
                </span>
              </div>
                  ...
                  <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                    ...
                    <div className="flex shrink-0 gap-1.5">
                      <Button
                        ...
                        onClick={() =>
                          act(
                            () => setStatus.mutateAsync({ id: r.id, status: "Approved" }),
                            "Review approved",
                          )
                        }
                      >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          act(
                            () => setStatus.mutateAsync({ id: r.id, status: "Rejected" }),
                            "Review rejected",
                          )
                        }
                      >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          act(
                            () => setStatus.mutateAsync({ id: r.id, status: "Hidden" }),
                            "Review hidden",
                          )
                        }
                      >
