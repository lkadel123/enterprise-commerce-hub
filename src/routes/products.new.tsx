...
function NewProduct() {
  ...
  const submit = (productStatus: "Active" | "Draft") => {
    ...
    productsApi
      ...
      .then(() => {
        toast.success(productStatus === "Draft" ? "Draft saved" : "Product published to storefront");
        void navigate({ to: "/products" });
      })
      .catch((e: unknown) => toast.error(e instanceof AdminApiError ? e.message : "Could not save product"))
  ...
  return (
    <AppShell>
      ...
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" size="sm" className="h-9" disabled={saving} onClick={() => submit("Draft")}>
              Save draft
            </Button>
        ...
        <div className="space-y-4 xl:col-span-2">
          <Section title="Basic information" description="Names, identifiers and descriptions">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="SKU">
                <Input value={sku} onChange={(e) => { setSku(e.target.value); setDirty(true); }} placeholder="SKU-2201" className="h-9 num" />
              </Field>
              <Field label="Brand">
              ...
              <div className="sm:col-span-2">
                <Field label="Short description" hint="Displayed in listings and search results (max 160 characters).">
                  <Input value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} placeholder="Hot-swappable switches, aluminium chassis, USB-C" className="h-9" />
                </Field>
              </div>
              <div className="sm:col-span-2">
                <Field label="Full description">
                  <Textarea rows={5} value={fullDescription} onChange={(e) => setFullDescription(e.target.value)} placeholder="Describe materials, dimensions, warranty and key selling points." />
                </Field>
              </div>
            </div>
          </Section>
          ...
          <Section title="Pricing" description="Retail price, cost and tax handling">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <Field label="Regular price"><Input className="num h-9" value={price} onChange={(e) => { setPrice(e.target.value); setDirty(true); }} placeholder="189.00" /></Field>
              <Field label="Sale price"><Input className="num h-9" placeholder="159.00" /></Field>
              <Field label="Cost price"><Input className="num h-9" value={cost} onChange={(e) => { setCost(e.target.value); setDirty(true); }} placeholder="102.40" /></Field>
            ...
            >
              ...
              <label className="mt-3">
                ...
                <Button variant="outline" size="sm" className="h-8" asChild>
                  <span><ImagePlus className="h-4 w-4" /> Select files</span>
                </Button>
              </label>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {images.map((img, i) => (
                <div key={img.url} className="group relative aspect-square overflow-hidden rounded-md border bg-surface-muted">
                ...
                <tbody>
                  {variations.map((v) => (
                    <tr key={v.id} className="border-t">
                      <td className="px-3 py-2"><Input value={v.size} onChange={(e) => setVariations((s) => s.map((x) => (x.id === v.id ? { ...x, size: e.target.value } : x)))} className="h-8" placeholder="M" /></td>
                      <td className="px-3 py-2"><Input value={v.color} onChange={(e) => setVariations((s) => s.map((x) => (x.id === v.id ? { ...x, color: e.target.value } : x)))} className="h-8" placeholder="Graphite" /></td>
                      <td className="px-3 py-2"><Input value={v.sku} onChange={(e) => setVariations((s) => s.map((x) => (x.id === v.id ? { ...x, sku: e.target.value } : x)))} className="num h-8" /></td>
                      <td className="px-3 py-2"><Input value={v.price} onChange={(e) => setVariations((s) => s.map((x) => (x.id === v.id ? { ...x, price: e.target.value } : x)))} className="num h-8" placeholder="0.00" /></td>
                      <td className="px-3 py-2"><Input value={v.stock} onChange={(e) => setVariations((s) => s.map((x) => (x.id === v.id ? { ...x, stock: e.target.value } : x)))} className="num h-8" placeholder="0" /></td>
