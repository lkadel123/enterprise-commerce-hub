...
// Verify-request observability: when the storefront POSTs the verification,
// decode the SIGNED response token's PAYLOAD SHAPE — key paths only, plus
// amount/currency values (merchant data). The token string itself, card data
// and any credentials are NEVER printed.
page.on("request", (req) => {
  if (!/\/customer\/payments\/.+\/verify$/.test(req.url()) || req.method() !== "POST") return;
  try {
    ...
    if (typeof token === "string" && token.split(".").length === 3) {
      ...
      const walk = (v, p, d) => {
        ...
        for (const [k, item] of Object.entries(v)) {
          ...
          if (item !== null && typeof item === "object") {
            paths.push(path);
            walk(item, path, d + 1);
          } else if (/^(totalAmount|amount|currency|taxAmount)$/i.test(k)) {
            paths.push(`${path}=${String(item)}`);
          } else {
            paths.push(path);
          }
        }
      ...
      walk(payload, "", 0);
      log("VERIFY TOKEN SHAPE (key paths + amounts):", JSON.stringify(paths));
    } else {
      log("VERIFY BODY keys (no JWT):", Object.keys(body ?? {}).join(","));
    }
  } catch (e) {
    log("VERIFY BODY capture failed:", String(e).slice(0, 120));
  }
});
...
try {
  ...
  // The backend enforces the literal `acceptedTerms: true`; the UI gates submission
  // on the Radix terms checkbox (role=checkbox, named via aria-labelledby).
  await page.getByRole("checkbox").first().check({ timeout: 15_000 });
  ...
  // Payment-method audit: prove the browser offers EXACTLY COD / Fonepay QR /
  // Card (no PayBridge / Khalti / eSewa / Bank Transfer).
  const offeredMethods = await page.evaluate(() =>
    Array.from(document.querySelectorAll("input[type=radio], [role=radio]")).map(
      (el) =>
  ...
  log("PAYMENT METHODS OFFERED:", JSON.stringify(offeredMethods));
  await page.screenshot({ path: SHOT("2b-payment-step"), fullPage: true });
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const state = await page.evaluate(() => {
      ...
      return {
        ...
        // Phase 21 diagnostic: the panel renders its caught error as role=alert
        // and its action button label is the only way to tell which branch ran.
        alertText: document.querySelector("[role='alert']")?.textContent?.slice(0, 240) ?? null,
        buttons: Array.from(document.querySelectorAll("button"))
          .map((b) => (b.textContent ?? "").trim().slice(0, 40))
          .filter(Boolean)
          .slice(0, 10),
        // Phase 21 diagnostic (read-only): did the Cybersource SDK actually
        // publish its constructor global on `window`? Distinguishes a
        // publish-timing race from "the script never executed".
        sdk: (() => {
          const script = document.getElementById("cybersource-unified-checkout-sdk");
          const vas = window.VAS;
          return {
            vas: typeof vas,
            vasKeys:
              vas && typeof vas === "object" ? Object.keys(vas).slice(0, 8) : null,
            unifiedPayments: typeof window.UnifiedPayments,
            scriptPresent: Boolean(script),
            integrity: script ? Boolean(script.getAttribute("integrity")) : null,
            srcTail: script ? (script.getAttribute("src") ?? "").slice(-42) : null,
          };
        })(),
      };
    });
...
const clickButtonInFrames = async (nameRe, label) => {
  ...
  // NOTE: the section-7 snapshot loop breaks as soon as ANY field exists, so
  // `cardFields` can predate the MCE email step mounting. Collect a FRESH
  // field snapshot (waiting up to ~15s for the email step) before deciding
  // which step of the flow we are on.
  ...
  const isEmailField = (f) => /^email$/i.test(f.auto) || f.name === "billTo.email" || f.id === "contact-email";
  let allFields = cardFields.flatMap((e) => e.fields.map((f) => ({ frame: e.frame, f })));
  for (let i = 0; i < 10 && !allFields.some(({ f }) => isEmailField(f)); i++) {
    await page.waitForTimeout(1_500);
    allFields = (await collectFields()).flatMap((e) => e.fields.map((f) => ({ frame: e.frame, f })));
  }
  const sel = (f) => {
    const s = [];
    if (f.id) s.push(`#${f.id}`);
  ...
  const emails = allFields.filter(({ f }) => isEmailField(f));
  const cards = allFields.filter(({ f }) =>
    /cc-number|cardNumber|expiry|expir|cvv|security code|cc-csc/i.test(`${f.auto} ${f.name} ${f.id} ${f.ph}`) && !isEmailField(f),
  );

  // Always fill the email when the field is present: the two-step flow
  // requires it before the card step is reachable, and on a one-page form it
  // is a required field too. Submitting Continue on an empty email step fails
  // client-side validation SILENTLY (no navigation, no SCA call).
  if (emails.length > 0) {
  ...
  // 8b. Poll until the card-entry step appears (card / payment fields), then
  // snapshot. `collectFields` is defined above (shared with the email step).
  let snapshot = [];
  for (let i = 0; i < 12; i++) {
    await page.waitForTimeout(1_500);
...
log("FILL REPORT:", JSON.stringify(fillReport));
  ...
  // TWO PASSES: payment buttons first across ALL frames, then generic
  // continue/submit — otherwise a review frame's "Continue" (rendered in a
  // frame iterated BEFORE the card form's frame) swallows the click and the
  // payment is never submitted (mount() then never resolves, no verify call).
  let clicked = false;
  const clickFirstMatch = async (patterns) => {
    for (const pattern of patterns) {
      for (const frame of page.frames()) {
        ...
        try {
          ...
          if ((await btn.count()) > 0) {
            await btn.first().click({ timeout: 3_000 });
            log("clicked UC payment button:", String(pattern), "in", frame.url().slice(0, 80));
            return true;
          }
        } catch {
          /* next frame */
  ...
  clicked =
    (await clickFirstMatch([/pay/i, /complete purchase/i, /submit payment/i, /place order/i])) ||
    (await clickFirstMatch([/continue/i, /submit/i]));
  if (!clicked) log("no UC pay button found to click");

  // The card-entry flow has a FINAL REVIEW step ("Confirm and Continue") that
  // appears a moment after the card form submits. Without clicking it the sale
  // never processes and mount() never resolves. Poll for it across frames.
  let confirmed = false;
  for (let i = 0; i < 12 && !confirmed; i++) {
    await page.waitForTimeout(1_500);
    confirmed = await clickFirstMatch([/confirm and continue/i, /^confirm$/i]);
  }
  log(confirmed ? "clicked UC review confirm button" : "no review confirm step appeared");
  ...
  try {
    await Promise.race([
      page.getByText(/payment successful|payment received/i).first().waitFor({ timeout: 180_000 }).then(() => { outcome = "successful"; }),
  ...
  // 10. COD leg — a second order in the SAME session, settled without any
  //     gateway transaction. Verifies the COD path end-to-end in the browser.
  try {
    await page.goto(`${BASE}/products`);
    await page.locator('a[href^="/products/"]').first().waitFor({ timeout: 30_000 });
    await page.locator('a[href^="/products/"]').first().click();
    ...
    await page.getByText(/added to cart/i).waitFor({ timeout: 15_000 });
    await page.goto(`${BASE}/checkout`);
    // Address step only appears when no address has been entered yet.
    if (await page.getByLabel("Address line 1").isVisible().catch(() => false)) {
      await page.getByLabel("Address line 1").fill("1 Live Lab Way");
      await page.getByLabel("City").fill("Kathmandu");
      await page.getByLabel("Postal code").fill("44600");
      await page.getByRole("button", { name: /continue to payment/i }).click();
    }
    await page.getByRole("radio", { name: /cash on delivery/i }).check({ timeout: 20_000 });
    await page.getByRole("button", { name: /review order/i }).click();
    await page.getByRole("button", { name: /place order/i }).click();
    await page.waitForURL(/order-confirmation/, { timeout: 30_000 });
    const codOrderId = page.url().split("/").pop();
    log("COD order placed:", codOrderId);
    const codPayment = await page.evaluate(async (id) => {
      const res = await fetch(`/api/v1/customer/payments/${id}/status`, { credentials: "include" });
      return { http: res.status, body: await res.json().catch(() => null) };
    }, codOrderId);
    log("COD PAYMENT RECORD:", JSON.stringify(codPayment).slice(0, 600));
    await page.waitForTimeout(1_500);
    await page.screenshot({ path: SHOT("7-cod"), fullPage: true });
  } catch (codError) {
    log("COD LEG ERROR:", String(codError).slice(0, 400));
    await page.screenshot({ path: SHOT("7-cod-error"), fullPage: true }).catch(() => undefined);
  }
